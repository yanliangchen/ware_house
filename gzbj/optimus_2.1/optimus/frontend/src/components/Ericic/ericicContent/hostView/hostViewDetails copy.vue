      ...
      <div class="content">
        <div class="cpu-content">
          <div class="socket_info">
            <div class="title">
              <span v-for="(item,index) in socketList"  :key="index">{{item}}</span>
            </div>
            <ul class="left" >
              <li v-for="(item,index) in coreList"  :key="index">{{item}}</li>
            </ul>
            <ul class="right" >
              <li v-for="(item,index) in socketNumList"  :key="index">
                <span>
                  [
                ...
                <!-- <span style="color:red;font-weight:900" v-if="item.type">{{item.name}}</span>
  ...
  methods:{
    ...
    async getLayoutData(){
      ...
      if(code == 200 && data){
        ...
        this.instanceData = tempInstance;

        let redCpu = [],tempRedcpu=[];
        tempInstance.forEach(res=>{
        ...
        if(newTempCore1.length>0){
          ...
              }
            ...
            this.socketNumList =  tempNumList;
          }else{
            this.socketNumList =  tempNumList;
          }
        }
